...
const ProductInfo=()=> {
    ...
    const getProductData = async () => {
        setLoading(true)
        try {
            const productTemp = await getDoc(doc(data, "product", params.id))
            // console.log(productTemp)
            setProducts(productTemp.data());

            // const resultref = await getDoc(doc(data, "products"))
            // const db =  query(resultref)
            // const querySnapShot = await getDoc(db)
            // const main = querySnapShot.doc.map((doc)=>({
            //   id:params.id, ...doc.data()
            // }))

            // const docRef = data.collection('products').doc(params.id)
            // const  main =await docRef.get()
            // // console.log(productTemp.data())
            // setProducts(main.data());
            setLoading(false)
        } catch (error) {
            console.log(error)
            setLoading(false)
            ...
            <div className=' flex w-full mt-10 '> 
                      <div className='flex flex-col gap-5 w-1/2 px-16'>
                             <div>
                              ...
                              <div>
                              <p className='text-5xl raleway text-[3rem] font-extrabold tracking-wide text-white'>PRODUCT INFO</p>
                              </div>
