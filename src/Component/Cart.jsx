...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    cartItems.map((cartItem) => {
      let cart = cartItem.price
      // console.log(cart)
      temp = temp + parseInt(cart)
    })
    setTotalAmount(temp);
    // console.log(temp)
  }, [cartItems])
  ...
  const buyNow = async () => {
    ...
    var options = {
      ...
      handler:  async function  (response) {
        ...
        try {
          const resultRef = collection(data, "orders")
           await addDoc(resultRef, orderInfo)
        } catch (error) {
          console.log(error)
        }
      },
    ...
    <MainLayout>
      ...
      <div className=' flex w-full mt-10 '> 
                <div className='flex flex-col gap-5 w-1/2 px-16'>
                       <div>
                        <p className='text-white  raleway text-[1.3rem] font-thin '>View</p>
                        </div> 
                        <div>
                        <p className='text-5xl raleway text-[3rem] font-extrabold tracking-wide text-white'>ALL CART ITEMS</p>
                        </div>
          
                    <div>
                    <p className='text-xl text-white mb-40'> The point of using Lorem Ipsum is that it has a </p>
                    </div>
...
<div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
  ...
  <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" >
    ...
    <Modal 
        name={name} address={address} pincode={pincode} phoneNumber={phoneNumber} setName={setName} setAddress={setAddress} setPincode={setPincode} setPhoneNumber={setPhoneNumber} buyNow={buyNow}
    />
  </div>
</div>
</div>
